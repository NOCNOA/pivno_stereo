...
def readDispSceneFlow(file_name):
    """Read dense SceneFlow disparity without imposing an evaluation range."""
    disp = readPFM(file_name).astype(np.float32)
    if disp.ndim != 2:
        raise ValueError(
            f"SceneFlow disparity must be a single-channel PFM, got {disp.shape}"
        )
...
def eth3d_disp_reader(filename):
    disp = frame_utils.read_gen(filename)

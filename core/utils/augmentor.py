...
def _photo_aug_numpy(photo_aug, image):
    image = np.array(image, copy=True)
    tensor = torch.from_numpy(image).permute(2, 0, 1).float() / 255.0
    tensor = photo_aug(tensor).clamp_(0.0, 1.0)
    return tensor.mul(255.0).round().to(torch.uint8).permute(1, 2, 0).contiguous().cpu().numpy()
...
class DispAugmentor:
    ...
    def color_transform(self, img1, img2):
        ...
        if np.random.rand() < self.asymmetric_color_aug_prob:
            img1 = _photo_aug_numpy(self.photo_aug, img1)
            img2 = _photo_aug_numpy(self.photo_aug, img2)

        # symmetric
        else:
            image_stack = np.concatenate([img1, img2], axis=0)
            image_stack = _photo_aug_numpy(self.photo_aug, image_stack)
    ...
    def spatial_transform(self, img1, img2, disp, valid=None):
        ...
        if np.random.rand() < self.spatial_aug_prob or min_scale >= 1.0:
            ...
            if valid is not None:
                valid = cv2.resize(valid.astype(np.uint8), None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_NEAREST) > 0
    ...
    def __call__(self, img1, img2, disp, valid=None):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        transformed = self.spatial_transform(img1, img2, disp, valid)

        if valid is None:
            img1, img2, disp = transformed
            img1 = np.ascontiguousarray(img1)
            img2 = np.ascontiguousarray(img2)
            disp = np.ascontiguousarray(disp)
            return img1, img2, disp

        img1, img2, disp, valid = transformed
        img1 = np.ascontiguousarray(img1)
        img2 = np.ascontiguousarray(img2)
        disp = np.ascontiguousarray(disp)

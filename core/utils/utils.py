...
def check_finite(x, name, step=None, itr=None):
    if isinstance(x, (list, tuple)):
        for i, t in enumerate(x):
            check_finite(t, f"{name}[{i}]", step=step, itr=itr)
        return
    ...
    if not torch.isfinite(x).all():
        ...
        print("=" * 80, flush=True)
        print(f"[BAD TENSOR] {name}", flush=True)
        if step is not None:
            print(f"step = {step}", flush=True)
        if itr is not None:
            print(f"itr = {itr}", flush=True)
        print(f"shape = {tuple(x.shape)}", flush=True)
        print(f"dtype = {x.dtype}", flush=True)
        print(f"nan_count = {nan_count}", flush=True)
        print(f"inf_count = {inf_count}", flush=True)

        x_safe = torch.nan_to_num(x.detach(), nan=0.0, posinf=1e6, neginf=-1e6)
        print(f"min = {x_safe.min().item()}", flush=True)
        print(f"max = {x_safe.max().item()}", flush=True)
        print(f"mean = {x_safe.float().mean().item()}", flush=True)
        print("=" * 80, flush=True)

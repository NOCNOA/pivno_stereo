"""Standalone RGB DEFOM-PIVNO with gated encoded matches and 3x3 GRUs."""
...
class DEFOMStereo(nn.Module):
    """Independent PIVNO model with scale gating and spatial outer GRUs."""

    MODEL_VARIANT = "defom_pivno_gated_gru3"
    SCALE_GATE_MODE = "gwc4_mean_softmax_weighted_encoded_concat"
    GRU_KERNEL_SIZE = IGEVStyleBasicMultiUpdateBlock.GRU_KERNEL_SIZE
    MATCH_NUM_GROUPS = 8 #4
    ...
    def _refine_warp_feature(
        ...
        del sampled_right
        group_correlation = encoded_right[
            :, :, :, -self.MATCH_NUM_GROUPS:
        ]
        scale_weights = self._apply_scale_gate(
            fmap1_low,
            group_correlation,
        )
        del group_correlation

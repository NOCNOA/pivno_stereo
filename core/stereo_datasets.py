...
class StereoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.is_eval and len(self.disparity_list[index]) > 1 and np.random.rand() > 0.5:
            ...
            img1 = np.asarray(img1, dtype=np.uint8)[:, ::-1]
            img2 = np.asarray(img2, dtype=np.uint8)[:, ::-1]
            disp = np.asarray(disp, dtype=np.float32)[:, ::-1]
            valid = np.asarray(valid, dtype=np.bool_)[:, ::-1]
        ...
        img1 = torch.from_numpy(np.ascontiguousarray(img1)).permute(2, 0, 1).float()
        img2 = torch.from_numpy(np.ascontiguousarray(img2)).permute(2, 0, 1).float()
        disp = torch.from_numpy(np.ascontiguousarray(disp[..., np.newaxis])).permute(2, 0, 1).float()
        valid = torch.from_numpy(np.ascontiguousarray(valid[..., np.newaxis].astype(np.bool_))).permute(2, 0, 1)
...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root=None, dstype='frames_cleanpass', things_test=False):
        super(SceneFlowDatasets, self).__init__(
            aug_params,
            reader=frame_utils.readDispSceneFlow,
            is_eval=things_test,
        )
        self.root = root or os.environ.get(
            "SCENEFLOW_ROOT", "/home/share/yijiayi/sceneflow/"
        )
        self.dstype = dstype
...
class ETH3D(StereoDataset):
    def __init__(self, aug_params=None, root=None, split='training', is_eval=False, is_test=False):
        super(ETH3D, self).__init__(aug_params, sparse=True, is_eval=is_eval, is_test=is_test, reader = frame_utils.eth3d_disp_reader )
        root = root or os.environ.get(
            "ETH3D_ROOT", "/home/share/yijiayi/eth3d"
        )
        if not osp.isdir(root):
            raise FileNotFoundError(
                f"ETH3D dataset not found: {root}. Set ETH3D_ROOT to the "
                "directory containing two_view_training and two_view_training_gt."
            )
        #/home/share/yijiayi/eth3d/two_view_training/delivery_area_1l/im0.png   frame_utils.eth3d_disp_reader
        ...
        # disp_list = sorted(glob(osp.join(root, 'two_view_training_gt/*/mask0occ.pfm'))) if split == 'training'\
        #     else [osp.join(root, 'two_view_training_gt/playground_1l/mask0occ.pfm')]*len(image1_list)
        # image1_list = sorted(glob(osp.join(root, f'two_view_training/delivery_area_1l/im0.png')))
        # image2_list = sorted(glob(osp.join(root, f'two_view_training/delivery_area_1l/im1.png')))
        # disp_list = sorted(glob(osp.join(root, 'two_view_training_gt/delivery_area_1l/disp0GT.pfm'))) if split == 'training'\
        #     else [osp.join(root, 'two_view_training_gt/playground_1l/disp0GT.pfm')]*len(image1_list)
        if not (len(image1_list) == len(image2_list) == len(disp_list) > 0):
            raise FileNotFoundError(
                "Incomplete ETH3D split under "
                f"{root}: left={len(image1_list)}, right={len(image2_list)}, "
                f"disparity={len(disp_list)}"
            )
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root=None, split='15', image_set='training', is_eval=False, is_test=False):
        ...
        base_root = root or os.environ.get(
            "KITTI_ROOT", "/home/share/yijiayi/kitti/"
        )
        year = {"12": "2012", "15": "2015"}[split]
        candidates = [
            osp.join(base_root, year),
            osp.join(base_root, split),
        ]
        root = next((path for path in candidates if osp.isdir(path)), None)
        if root is None:
            raise FileNotFoundError(
                f"KITTI-{year} dataset not found; checked {candidates}. "
                "Set KITTI_ROOT to the directory containing 2012 and 2015."
            )
...
class Middlebury(StereoDataset):
    def __init__(self, aug_params=None, root=None, split='F', image_set='training', is_eval=False, is_test=False):
        ...
        root = root or os.environ.get(
            "MIDDLEBURY_ROOT", "/home/share/yijiayi/Middlebury"
        )
        if not osp.isdir(root):
            raise FileNotFoundError(
                f"Middlebury dataset not found: {root}. Set MIDDLEBURY_ROOT "
                "to the directory containing MiddEval3 and 2021."
            )
...
def fetch_dataloader(args):
    ...
    for fold, dataset_name in zip(args.train_folds, args.train_datasets):
        if dataset_name.startswith("middlebury_"):
            ...
            new_dataset = final_dataset * fold
            logging.info(
                f"Adding {len(new_dataset)} samples from SceneFlow finalpass"
            )

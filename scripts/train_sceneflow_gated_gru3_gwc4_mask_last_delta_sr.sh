...
# Keep the 200k base for a controlled comparison with the earlier SR heads.
# Override BASE_CKPT with the 190k checkpoint for an EPE-oriented follow-up.
BASE_CKPT="${BASE_CKPT:-checkpoints/defom_pivno_gated_gru3_gwc4gate_enc16_noleft_rgb_d768_320x768_b4_2gpu_200k.pth}"
NAME="${NAME:-defom_pivno_gated_gru3_gwc4_mask_last_delta_sr_head_d768_20k}"
...
echo "C32/GWC4 gated-GRU3 final-delta weighted SR launch"
echo "  base=${BASE_CKPT}"
echo "  name=${NAME} stage=head trainable=sr_head.*"
echo "  formula=base_final + weighted_refined_last_delta - mask_last_delta"
...
"${CONDA_BIN}" run -n "${CONDA_ENV}" --no-capture-output torchrun \
  ...
  train_stereo.py \
  --model defom_pivno_gated_gru3_gwc4_mask_last_delta_sr \
  --distributed \
  --launcher pytorch \
  --gpu_ids "${GPU_IDS[@]}" \
  --name "${NAME}" \

...
NAME="${NAME:-defom_pivno_gated_gru1_c48_gwc8_enc16_noleft_rgb_d768_${IMAGE_HEIGHT}x${IMAGE_WIDTH}_b${BATCH_SIZE}_${NUM_GPUS}gpu_200k}"
...
echo "Launching isolated DEFOM-PIVNO gated GRU1 from scratch: GPUs=${CUDA_VISIBLE_DEVICES}, world_size=${NUM_GPUS}, global_batch=${BATCH_SIZE}, local_batch=${LOCAL_BATCH_SIZE}, lr=${LEARNING_RATE}, steps=${NUM_STEPS}, crop=${IMAGE_HEIGHT}x${IMAGE_WIDTH}, iters=${TRAIN_ITERS}, data=${SCENEFLOW_ROOT}"

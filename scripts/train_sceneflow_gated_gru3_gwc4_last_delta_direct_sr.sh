...
NAME="${NAME:-defom_pivno_gated_gru3_gwc4_last_delta_direct_sr_head_d768_20k}"
...
echo "C32/GWC4 gated-GRU3 direct final-delta SR launch"
echo "  base=${BASE_CKPT}"
echo "  name=${NAME} stage=head trainable=sr_head.*"
echo "  inputs=final_hidden+disp_before_update+context_zqr+motion"
echo "  formula=previous_iteration_upsampled_disp+pixelshuffle_delta"
echo "  GPUs=${CUDA_VISIBLE_DEVICES} global_batch=${GLOBAL_BATCH_SIZE} local_batch=${LOCAL_BATCH_SIZE}"
echo "  crop=${IMAGE_HEIGHT}x${IMAGE_WIDTH} steps=${NUM_STEPS} lr=${LEARNING_RATE}"
...
"${CONDA_BIN}" run -n "${CONDA_ENV}" --no-capture-output torchrun \
  ...
  train_stereo.py \
  --model defom_pivno_gated_gru3_gwc4_last_delta_direct_sr \
  --distributed \
  --launcher pytorch \
  --gpu_ids "${GPU_IDS[@]}" \
  --name "${NAME}" \

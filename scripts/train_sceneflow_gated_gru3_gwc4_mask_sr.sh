...
MASTER_PORT="${MASTER_PORT:-29556}"

# The completed checkpoint is C32/GWC4/enc16 despite the later C48/GWC8
# source drift. The isolated model route restores the checkpoint shapes.
BASE_CKPT="${BASE_CKPT:-checkpoints/defom_pivno_gated_gru3_gwc4gate_enc16_noleft_rgb_d768_320x768_b4_2gpu_200k.pth}"
NAME="${NAME:-defom_pivno_gated_gru3_gwc4_mask_sr_head_d768_20k}"
...
# SR is final-only, so train and validation must expose it to the same state.
...
if [[ ! -f "${BASE_CKPT}" ]]; then
  echo "Completed GWC4 gated-GRU3 checkpoint does not exist: ${BASE_CKPT}" >&2
  exit 2
fi
...
echo "Completed C32/GWC4 gated-GRU3 mask-guided SR launch"
echo "  base=${BASE_CKPT}"
...
"${CONDA_BIN}" run -n "${CONDA_ENV}" --no-capture-output torchrun \
  ...
  train_stereo.py \
  --model defom_pivno_gated_gru3_gwc4_mask_sr \
  --distributed \
  --launcher pytorch \
  --gpu_ids "${GPU_IDS[@]}" \
  --name "${NAME}" \

...
# Train MobileNetV2 stereo features + bins + gated GRU3 jointly.
# MobileNetV2 starts from ImageNet weights when available; all other weights
# start from scratch. BatchNorm running statistics stay frozen.
...
NAME="${NAME:-defom_pivno_gated_gru3_mobilenetv2_bins_d${MAX_DISP}_${IMAGE_HEIGHT}x${IMAGE_WIDTH}_b${BATCH_SIZE}_200k}"
...
echo "Launching MobileNetV2 + bins + gated-GRU3 training:"
...
"${CONDA_BIN}" run -n "${CONDA_ENV}" --no-capture-output torchrun \
  ...
  --val_freq 20000 \
  "${EXTRA_ARGS[@]}" \
  --model defom_pivno_gated_gru3_mobilenetv2_bins \
